...
export class RequestBook extends Component{
    ...
    refreshList(){
        ...
        fetch(variables.API_URL+'requestbooks')
    ...
    updateClick(id){
        fetch(variables.API_URL+'requestbooks/'+id,{
            method:'PUT',
            headers:{
                'Accept':'application/json',
                'Content-Type':'application/json'
    ...
    <tbody>
        {request.map((req, i)=>
            <tr key={i}>
                ...
                <button type="button"
                className="btn btn-light mr-1"
                onClick={()=>this.deleteClick(req.id)}>
